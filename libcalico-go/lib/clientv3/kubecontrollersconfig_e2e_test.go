...
var _ = testutils.E2eDatastoreDescribe("KubeControllersConfiguration tests", testutils.DatastoreAll, func(config apiconfig.CalicoAPIConfig) {
	...
	DescribeTable("KubeControllersConfiguration e2e CRUD tests",
		func(spec1, spec2 apiv3.KubeControllersConfigurationSpec, status1, status2 apiv3.KubeControllersConfigurationStatus, configNames ...string) {
			if configNames == nil {
				Fail("Should specify at least one configName")
				return
			}
			for _, configName := range configNames {
				...
				Expect(outError.Error()).To(ContainSubstring("resource does not exist: KubeControllersConfiguration(" + configName + ") with error:"))
				...
				_, outError = c.KubeControllersConfiguration().Create(ctx, &apiv3.KubeControllersConfiguration{
					ObjectMeta: metav1.ObjectMeta{Name: configName, ResourceVersion: "12345"},
					Spec:       spec1,
				}, options.SetOptions{})
				Expect(outError).To(HaveOccurred())
				...
				_, outError = c.KubeControllersConfiguration().Get(ctx, configName, options.GetOptions{})
				Expect(outError).To(HaveOccurred())
				Expect(outError.Error()).To(ContainSubstring("resource does not exist: KubeControllersConfiguration(" + configName + ") with error:"))
			}
			...
			Expect(outError).To(HaveOccurred())
			Expect(outError.Error()).To(Equal("Cannot create a Kube Controllers Configuration resource with a name other than \"default\" and \"elasticsearch\""))
			...
			for _, configName := range configNames {
				...
				Expect(res1).To(MatchResource(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec1))
				...
				Expect(outError).NotTo(HaveOccurred())
				Expect(res).To(MatchResource(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec1))
				...
				Expect(outList.Items).To(ConsistOf(
					testutils.Resource(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec1),
				))
				...
				Expect(outError).NotTo(HaveOccurred())
				Expect(res1).To(MatchResource(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec2))
				...
				res, outError = c.KubeControllersConfiguration().Update(ctx, &apiv3.KubeControllersConfiguration{
					ObjectMeta: metav1.ObjectMeta{Name: configName, ResourceVersion: "1234", CreationTimestamp: metav1.Now()},
					Spec:       spec1,
				}, options.SetOptions{})
				Expect(outError).To(HaveOccurred())
				Expect(res).To(BeNil())
				...
				_, outError = c.KubeControllersConfiguration().Update(ctx, res1, options.SetOptions{})
				Expect(outError).To(HaveOccurred())
				Expect(outError.Error()).To(Equal("update conflict: KubeControllersConfiguration(" + configName + ")"))
				...
				if config.Spec.DatastoreType != apiconfig.Kubernetes {
					...
					res, outError = c.KubeControllersConfiguration().Get(ctx, configName, options.GetOptions{ResourceVersion: rv1_1})
					Expect(outError).NotTo(HaveOccurred())
					Expect(res).To(MatchResource(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec1))
				...
				res, outError = c.KubeControllersConfiguration().Get(ctx, configName, options.GetOptions{ResourceVersion: rv1_2})
				Expect(outError).NotTo(HaveOccurred())
				Expect(res).To(MatchResource(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec2))
				...
				Expect(outError).NotTo(HaveOccurred())
				Expect(outList.Items).To(ConsistOf(
					testutils.Resource(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec2),
				))
				...
				Expect(res).To(MatchResourceWithStatus(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec2, status1))
				...
				res, outError = c.KubeControllersConfiguration().Update(ctx, res, options.SetOptions{})
				Expect(outError).ToNot(HaveOccurred())
				Expect(res).To(MatchResourceWithStatus(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec2, status2))
				...
				if config.Spec.DatastoreType != apiconfig.Kubernetes {
					...
					_, outError = c.KubeControllersConfiguration().Delete(ctx, configName, options.DeleteOptions{ResourceVersion: rv1_1})
					Expect(outError).To(HaveOccurred())
					Expect(outError.Error()).To(Equal("update conflict: KubeControllersConfiguration(" + configName + ")"))
				}

				By("Deleting KubeControllersConfiguration with the new resource version")
				dres, outError := c.KubeControllersConfiguration().Delete(ctx, configName, options.DeleteOptions{ResourceVersion: rv1_3})
				Expect(outError).NotTo(HaveOccurred())
				Expect(dres).To(MatchResourceWithStatus(apiv3.KindKubeControllersConfiguration, testutils.ExpectNoNamespace, configName, spec2, status2))
			}
		...
		Entry("Two fully populated KubeControllersConfigurationSpecs for only calico kube-controller", spec1, spec2, status1, status2, calicoKubeControllerConfigName),
		Entry("Two fully populated KubeControllersConfigurationSpecs for only es kube-controller", spec1, spec2, status1, status2, esKubeControllerConfigName),
		Entry("Two fully populated KubeControllersConfigurationSpecs for both kube-controllers", spec1, spec2, status1, status2, calicoKubeControllerConfigName, esKubeControllerConfigName),
	)
		...
		It("should handle watch events for different resource versions and event types", func() {
			...
			outRes1, err := c.KubeControllersConfiguration().Create(
				...
				&apiv3.KubeControllersConfiguration{
					ObjectMeta: metav1.ObjectMeta{Name: calicoKubeControllerConfigName},
					Spec:       spec1,
				},
				options.SetOptions{},
			)
			...
			By("Deleting res1")
			_, err = c.KubeControllersConfiguration().Delete(ctx, calicoKubeControllerConfigName, options.DeleteOptions{})
			Expect(err).NotTo(HaveOccurred())
			...
			if config.Spec.DatastoreType == apiconfig.EtcdV3 {
				...
				w, err = c.KubeControllersConfiguration().Watch(ctx, options.ListOptions{Name: calicoKubeControllerConfigName, ResourceVersion: rev0})
				Expect(err).NotTo(HaveOccurred())
