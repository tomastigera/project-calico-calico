...
var _ = testutils.E2eDatastoreDescribe("PacketCapture tests", testutils.DatastoreAll, func(config apiconfig.CalicoAPIConfig) {
	...
	DescribeTable("PacketCapture e2e CRUD tests",
		func(name1, name2 string, spec1, spec2 apiv3.PacketCaptureSpec) {
			...
			Expect(outError).To(HaveOccurred())
			Expect(outError.Error()).To(ContainSubstring("resource already exists: PacketCapture(" + namespace1 + "/" + name1 + ")"))
			...
			if config.Spec.DatastoreType != apiconfig.Kubernetes {
				...
				Eventually(func() string {
					_, err := c.PacketCaptures().Get(ctx, namespace2, name2, options.GetOptions{})
					if err != nil {
						return err.Error()
					}
					return ""
				}, 5*time.Second, 200*time.Millisecond).Should(ContainSubstring("resource does not exist: PacketCapture(" + namespace2 + "/" + name2 + ") with error:"))
