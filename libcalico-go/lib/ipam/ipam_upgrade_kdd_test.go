...
// createUnlabeledBlockAffinity creates a BlockAffinity CRD directly through the CRD REST client
// without any labels. Name must be host + "-" + CIDRToName to match Calico conventions.
func createUnlabeledBlockAffinity(ctx context.Context, rc rest.Interface, host string, cidr string) error {
	...
	ba := &internalapi.BlockAffinity{
		...
		Spec: internalapi.BlockAffinitySpec{
			State:   string(v3.StateConfirmed),
			Node:    host,
			Type:    "host",
			CIDR:    cidr,
			Deleted: "false",
