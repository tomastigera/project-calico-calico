...
var _ = testutils.E2eDatastoreDescribe("IPAM tests", testutils.DatastoreAll, func(config apiconfig.CalicoAPIConfig) {
	...
	Describe("With an AWS-backed pool", func() {
		var hostname string
		poolCIDR := cnet.MustParseCIDR("10.0.0.0/26")
		...
		Describe("with a normal pool too", func() {
			pool2CIDR := cnet.MustParseCIDR("10.0.1.0/26")
	...
	DescribeTable("AutoAssign: requested IPs vs returned IPs",
		func(host string, cleanEnv bool, pools []ipamtestutils.Pool, usePool string, inv4, inv6 int, expv4ia, expv6ia *IPAMAssignments, blockLimit int, strictAffinity bool, expError error) {
			...
			if len(parts) > 1 {
				...
				case "host":
					use = v3.IPPoolAllowedUseHostSecondary
				default:
		...
		Entry("allowed use: requesting workload IP from host pool should fail", "test-host", true, []ipamtestutils.Pool{
			{CIDR: "192.168.1.0/24", BlockSize: 32, Enabled: true, AllowedUses: []v3.IPPoolAllowedUse{v3.IPPoolAllowedUseHostSecondary}},
...
func applyPoolWithAWSSubnet(cidr string, enabled bool, nodeSelector string, awsSubnetID string) {
	ipPools.Pools[cidr] = ipamtestutils.Pool{Enabled: enabled, NodeSelector: nodeSelector, BlockSize: 32, AWSSubnetID: awsSubnetID}
}
