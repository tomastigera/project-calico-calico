...
func NewKubeClient(ca *apiconfig.CalicoAPIConfigSpec) (api.Client, error) {
	...
	k8sAdminPolicyClient, err := adminNetworkPolicyClient(config)
	if err != nil {
		return nil, fmt.Errorf("Failed to build AdminNetwworkPolicy client: %v", err)
	}
	...
	registerEnterpriseResources(c, ca, restClient, group)
...
// adminNetworkPolicyClient builds a RESTClient configured to interact (Baseline) Admin Network Policy.
func adminNetworkPolicyClient(cfg *rest.Config) (*adminpolicyclient.PolicyV1alpha1Client, error) {
