...
func RulesAPIV3ToBackend(ars []apiv3.Rule, ns string, matchSGs bool) []model.Rule {
	if len(ars) == 0 {
		return nil
	}
	...
	for idx, ar := range ars {
		brs[idx] = RuleAPIV3ToBackend(ar, ns, matchSGs)
	}
	return brs
}
...
func RuleAPIV3ToBackend(ar apiv3.Rule, ns string, matchSGs bool) model.Rule {
