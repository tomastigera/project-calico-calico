...
func ConvertProfileV3ToV1Value(val any) (*model.Profile, error) {
	...
	for _, irule := range v3res.Spec.Ingress {
		irules = append(irules, RuleAPIV3ToBackend(irule, "", false))
	}

	var erules []model.Rule
	for _, erule := range v3res.Spec.Egress {
		erules = append(erules, RuleAPIV3ToBackend(erule, "", false))
	}
