...
type Aggregator struct {
	...
	previous                AggregationKind
	initial                 AggregationKind
...
func (a *Aggregator) CurrentAggregationLevel() AggregationKind {
	return a.current
}

func (a *Aggregator) DefaultAggregationLevel() AggregationKind {
	return a.initial
}

func (a *Aggregator) AggregationLevelChanged() bool {
	return a.current != a.previous
}

func (a *Aggregator) AdjustLevel(newLevel AggregationKind) {
	if a.current != newLevel {
		var value = newLevel
		...
		a.current = value
		log.Debugf("New aggregation level for %v is set to %d from %d", a.handledAction, a.current, a.previous)
	}
}

func (a *Aggregator) AggregateOver(kind AggregationKind) *Aggregator {
	a.initial = kind
	a.current = kind
	a.previous = kind
	return a
...
func (a *Aggregator) IncludeTcpStats(b bool) *Aggregator {
	a.includeTcpStats = b
	return a
}
...
func (a *Aggregator) IncludeProcess(b bool) *Aggregator {
	a.includeProcess = b
	return a
}
...
func (a *Aggregator) MaxDomains(s int) *Aggregator {
	a.maxDomains = s
	return a
}
...
func (a *Aggregator) PerFlowProcessLimit(n int) *Aggregator {
	a.perFlowProcessLimit = n
	return a
}

func (a *Aggregator) PerFlowProcessArgsLimit(n int) *Aggregator {
	a.perFlowProcessArgsLimit = n
	return a
}
...
func (a *Aggregator) FeedUpdate(mu *metric.Update) error {
	...
	if !ok {
		...
		spec := NewFlowSpec(mu, a.maxOriginalIPsSize, a.maxDomains, a.includeProcess, a.perFlowProcessLimit,
			a.perFlowProcessArgsLimit, a.displayDebugTraceLogs, a.natOutgoingPortLimit)
...
func (a *Aggregator) GetAndCalibrate(newLevel AggregationKind) []*FlowLog {
