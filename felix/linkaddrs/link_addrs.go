...
func WithIgnoredAddrs(c []ip.Addr) Option {
	return func(la *LinkAddrsManager) {
		la.wlAddressIgnoreList = c
	}
}
	...
	return la.ifaceNameToAddrs.Dataplane().ReplaceAllIter(func(f func(k string, v ip.CIDR)) error {
		...
		for name, addrs := range linkAddrMap {
			for _, addr := range addrs {
				...
				if la.addrInIgnoreList(ip.FromNetIP(addr.IP)) {
					// ignore address which is in the ignore list.
					log.Debugf("Ignoring address %s due to ignore list.", addr.IPNet.String())
					continue
				}
...
func (la *LinkAddrsManager) ensureLinkLocalAddress(nl netlinkshim.Interface, name string, newipCIDR ip.CIDR) error {
	...
	for _, netlinkAddr := range addrs {
		...
		if la.addrInIgnoreList(ip.FromNetIP(netlinkAddr.IP)) {
			log.Debugf("Ignoring address %s due to ignore list.", netlinkAddr.IPNet.String())
			continue
		}
