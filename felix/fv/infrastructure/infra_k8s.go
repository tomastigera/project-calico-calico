...
func createRemoteK8sDatastoreInfra(opts ...CreateOption) DatastoreInfra {
	return createK8sDatastoreInfraWithIndex(K8sInfraRemoteCluster, opts...)
}
...
func (kds *K8sDatastoreInfra) GetRemoteClusterConfig() *api.RemoteClusterConfiguration {
	return &api.RemoteClusterConfiguration{
		ObjectMeta: metav1.ObjectMeta{
			Name: "k8s-fv-remote",
		},
...
func cleanupAllRemoteClusterConfigs(clientset *kubernetes.Clientset, calicoClient client.Interface) {
	log.Info("Cleaning up RemoteClusterConfigurations")
	rccs, err := calicoClient.RemoteClusterConfigurations().List(context.Background(), options.ListOptions{})
	if err != nil {
		panic(err)
	}
	log.WithField("count", len(rccs.Items)).Info("RemoteClusterConfigurations present")
	for _, rcc := range rccs.Items {
		_, err := calicoClient.RemoteClusterConfigurations().Delete(context.Background(), rcc.Name, options.DeleteOptions{})
		if err != nil {
			panic(err)
		}
	}
	log.Info("Cleaned up RemoteClusterConfigurations")
}
...
func updatePodLabelsAndAnnotations(wep *libapi.WorkloadEndpoint, pod *v1.Pod) *v1.Pod {
	...
	if wep.Spec.EgressGateway != nil {
		...
		pod.Annotations["egress.projectcalico.org/egressGatewayPolicy"] = wep.Spec.EgressGateway.Policy
		if wep.Spec.EgressGateway.Gateway != nil {
			pod.Annotations["egress.projectcalico.org/selector"] = wep.Spec.EgressGateway.Gateway.Selector
			pod.Annotations["egress.projectcalico.org/namespaceSelector"] = wep.Spec.EgressGateway.Gateway.NamespaceSelector
		}
