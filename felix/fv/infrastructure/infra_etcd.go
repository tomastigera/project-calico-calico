...
func (eds *EtcdDatastoreInfra) AddNode(felix *Felix, v4CIDR *net.IPNet, v6CIDR *net.IPNet, idx int, needBGP bool) {
	...
	if felix.ExpectedWireguardTunnelAddr != "" {
		felixNode.Spec.Wireguard = &internalapi.NodeWireguardSpec{
...
func (eds *EtcdDatastoreInfra) GetRemoteClusterConfig() *api.RemoteClusterConfiguration {
	return &api.RemoteClusterConfiguration{
		...
		Spec: api.RemoteClusterConfigurationSpec{
			...
			EtcdConfig: api.EtcdConfig{
				EtcdEndpoints: "http://" + eds.EtcdContainer.IP + ":2379",
			},
		},
	}
}
