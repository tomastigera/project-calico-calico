...
func applyStateTransition(idx *SelectorAndNamedPortIndex, rec *testRecorder, s1, s2 namedPortState, strategy string) {
	...
	for k, ep := range s2.Endpoints {
		...
		ops = append(ops, func() {
			...
			idx.UpdateEndpointOrSet(k, uniquelabels.Make(ep.Labels), ep.CIDRs(), ep.Ports, ep.Parents, nil, time.Time{}, 0)
		})
	}
...
var _ = Describe("SelectorAndNamedPortIndex", func() {
	...
	Describe("NetworkSet domains", func() {
		It("should normalize domain names to lowercase", func() {
			...
			s, err := selector.Parse("villain == 'ghost'")
			Expect(err).ToNot(HaveOccurred())
			uut.UpdateIPSet("dvillains", s, ipsetmember.ProtocolNone, "")
			set, ok := recorder.ipsets["dvillains"]
			Expect(ok).To(BeTrue())
			Expect(set).To(HaveLen(2))
			Expect(set).To(HaveKey(ipsetmember.MakeDomain("microsoft.com")))
			Expect(set).To(HaveKey(ipsetmember.MakeDomain("mixed.org")))
		})
	})
	...
	Describe("Egress IP", func() {
		It("should only match an egress IP set against workload endpoints", func() {
			...
			s, err := selector.Parse("villain == 'ghost'")
			Expect(err).ToNot(HaveOccurred())
			uut.UpdateIPSet("e:villains", s, ipsetmember.ProtocolNone, "")
			set, ok := recorder.ipsets["e:villains"]
			Expect(ok).To(BeTrue())
			Expect(set).To(HaveLen(1))
			cidr, err := ip.CIDRFromString("192.168.4.10/32")
			Expect(err).NotTo(HaveOccurred())
			Expect(set).To(HaveKey(ipsetmember.MakeEgressGateway(
				cidr.Addr().(ip.V4Addr),
				time.Time{},
				0,
				"test-host",
