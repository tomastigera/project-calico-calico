...
func (d *endpointData) Equals(other *endpointData) bool {
	...
	if d.deletionGracePeriodSeconds != other.deletionGracePeriodSeconds {
		return false
	}
	...
	case model.WorkloadEndpointKey:
		if update.Value != nil {
			...
			idx.UpdateEndpointOrSet(
				...
				endpoint.DeletionGracePeriodSeconds,
			)
		} else {
...
func (idx *SelectorAndNamedPortIndex) UpdateIPSet(ipSetID string, sel *selector.Selector, namedPortProtocol ipsetmember.Protocol, namedPort string) {
	...
	newIPSetData := &ipSetData{
		...
		isEgressSelector:  strings.HasPrefix(ipSetID, "e"),
	}
...
func (idx *SelectorAndNamedPortIndex) UpdateEndpointOrSet(
	...
	newEndpointData.deletionGracePeriodSeconds = deletionGracePeriodSeconds
	...
	idx.scanEndpointAgainstIPSets(id, newEndpointData, oldIPSetContributions)
...
func (idx *SelectorAndNamedPortIndex) UpdateParent(parentData *npParentData, applyUpdate, revertUpdate func()) {
	parentData.IterEndpointIDs(func(id any) error {
		...
		applyUpdate()
		idx.scanEndpointAgainstIPSets(id, epData, oldIPSetContributions)
...
func (idx *SelectorAndNamedPortIndex) CalculateEndpointContribution(id any, d *endpointData, ipSetData *ipSetData) (contrib []ipsetmember.IPSetMember) {
	if ipSetData.namedPortProtocol != ipsetmember.ProtocolNone {
		...
		for _, domain := range d.domains {
			contrib = append(contrib, ipsetmember.MakeDomain(strings.ToLower(domain)))
		}
	} else {
			...
			for _, addr := range d.nets {
				if ipSetData.isEgressSelector {
					if addr.Version() == 4 {
						...
						member := ipsetmember.MakeEgressGateway(
							addr.Addr().(ip.V4Addr),
							d.deletionTimestamp,
							d.deletionGracePeriodSeconds,
							wep.Hostname,
							healthPort,
						)
