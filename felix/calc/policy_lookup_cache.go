...
func (pc *PolicyLookupsCache) updateRulesNFLOGPrefixes(
	...
	if model.KindIsStaged(kind) {
		...
		pc.addNFLogPrefixEntry(
			prefix,
			NewRuleID(kind, tier, name, namespace, RuleIndexTierDefaultAction, rules.RuleDirIngress, tierDefaultAction),
		)
		newPrefixes.Add(prefix)
		...
		pc.addNFLogPrefixEntry(
			prefix,
			NewRuleID(kind, tier, name, namespace, RuleIndexTierDefaultAction, rules.RuleDirEgress, tierDefaultAction),
		)
		newPrefixes.Add(prefix)
		log.Debugf("Updated staged policy no-match prefix: %s to default action: %v", prefix, tierDefaultAction)
	}
...
func (pc *PolicyLookupsCache) updateTierDefaultAction(key model.TierKey, tier *model.Tier) {
	...
	for polKey := range pc.nflogPrefixesPolicy {
		polTier := pc.keyToTier[polKey]
		if polTier != key.Name && model.KindIsStaged(polKey.Kind) {
			// Only evaluate policies that belong to this tier.
			continue
		}
		id := &types.PolicyID{Name: polKey.Name, Namespace: polKey.Namespace, Kind: polKey.Kind}
		...
		// the EOT is stored as a PPI and PPE, we can set the prefix corresponding to the
		// defaultAction. Change this and the egress prefix calculation.
		perfix := rules.CalculateNoMatchPolicyNFLOGPrefixStr(rules.RuleDirIngress, id)
		pc.addNFLogPrefixEntry(
			perfix,
			NewRuleID(polKey.Kind, polTier, polKey.Name, polKey.Namespace, RuleIndexTierDefaultAction, rules.RuleDirIngress, tierDefaultAction),
		)

		perfix = rules.CalculateNoMatchPolicyNFLOGPrefixStr(rules.RuleDirEgress, id)
		pc.addNFLogPrefixEntry(
			perfix,
			NewRuleID(polKey.Kind, polTier, polKey.Name, polKey.Namespace, RuleIndexTierDefaultAction, rules.RuleDirEgress, tierDefaultAction),
		)
	}
