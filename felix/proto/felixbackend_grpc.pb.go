...
const (
	...
	PolicySync_ReportWAF_FullMethodName = "/felix.PolicySync/ReportWAF"
)
...
type PolicySyncClient interface {
	...
	ReportWAF(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[WAFEvent, WAFEventResult], error)
}
...
func (c *policySyncClient) ReportWAF(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[WAFEvent, WAFEventResult], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &PolicySync_ServiceDesc.Streams[1], PolicySync_ReportWAF_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[WAFEvent, WAFEventResult]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type PolicySync_ReportWAFClient = grpc.ClientStreamingClient[WAFEvent, WAFEventResult]
...
type PolicySyncServer interface {
	...
	// Report WAF events to Felix.
	ReportWAF(grpc.ClientStreamingServer[WAFEvent, WAFEventResult]) error
...
func _PolicySync_ReportWAF_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(PolicySyncServer).ReportWAF(&grpc.GenericServerStream[WAFEvent, WAFEventResult]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type PolicySync_ReportWAFServer = grpc.ClientStreamingServer[WAFEvent, WAFEventResult]
...
var PolicySync_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "ReportWAF",
			Handler:       _PolicySync_ReportWAF_Handler,
			ClientStreams: true,
		},
	},
