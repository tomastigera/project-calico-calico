...
func TestMaglevNATNodePortNoFWD(t *testing.T) {
	...
	runBpfTest(t, "calico_to_host_ep", nil, func(bpfrun bpfProgRunFn) {
		...
		payloadL := pktR.ApplicationLayer()
		Expect(payloadL).NotTo(BeNil())
		Expect(payloadL.Payload()).To(Equal(resposeDefault))
		Expect(pktR.TransportLayer().TransportFlow().Dst()).To(BeEquivalentTo(l4.(gopacket.TransportLayer).TransportFlow().Src()))
		Expect(pktR.TransportLayer().TransportFlow().Src()).To(BeEquivalentTo(l4.(gopacket.TransportLayer).TransportFlow().Dst()))
	})
