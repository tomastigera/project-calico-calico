...
func TestTCPResetIPv6(t *testing.T) {
	...
	runBpfUnitTest(t, "tcp_rst.c", func(bpfrun bpfProgRunFn) {
		...
		Expect(res.dataOut).To(HaveLen(74)) // eth(14) + ipv6(40) + tcp(20)
...
func checkTcpRst(pktR gopacket.Packet, ipv4 *layers.IPv4, tcp *layers.TCP, ack bool) {
	...
	Expect(ok).To(BeTrue())
	Expect(tcpR.RST).To(BeTrue())
	Expect(tcpR.FIN).To(BeFalse())
	Expect(tcpR.SYN).To(BeFalse())
	Expect(tcpR.URG).To(BeFalse())
	Expect(tcpR.ECE).To(BeFalse())
	Expect(tcpR.CWR).To(BeFalse())
	Expect(tcpR.NS).To(BeFalse())
	Expect(tcpR.PSH).To(BeFalse())
