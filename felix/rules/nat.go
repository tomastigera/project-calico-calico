...
func (r *DefaultRuleRenderer) makeNATOutgoingRuleIPTables(ipVersion uint8, protocol string, action generictables.Action) generictables.Rule {
	...
	if r.EgressIPEnabled && ipVersion == 4 {
		// When EgressIP is enabled, any packet leaving cluster from a pod with egress annotation will be directed to vxlan tunnel
		// device to be encapsulated. To preserve the source pod ip, no SNAT should be taken.
		match = match.NotOutInterface(r.EgressIPInterface)
	}
