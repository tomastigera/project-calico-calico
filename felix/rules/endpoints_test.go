...
var (
	...
	_ = Describe("Endpoints with DropActionOverride=LOGandDROP", endpointRulesTests(false, "LOGandDROP"))
	_ = Describe("Endpoints with DropActionOverride=ACCEPT", endpointRulesTests(false, "ACCEPT"))
)
	...
	return func() {
		...
		for _, trueOrFalse := range []bool{true, false} {
			...
			rrConfigNormalMangleReturn := Config{
				...
				DNSPolicyMode:            v3.DNSPolicyModeDelayDeniedPacket,
			...
			// added.
			for _, dm := range []v3.DNSPolicyMode{v3.DNSPolicyModeNoDelay, v3.DNSPolicyModeDelayDNSResponse} {
				dnsMode := dm
				Context("with normal config and DNSMode set to NoDelay", func() {
					...
					AfterEach(func() {
						rrConfigNormalMangleReturn.DNSPolicyMode = v3.DNSPolicyModeDelayDeniedPacket
					})
...
var _ = table.DescribeTable("PolicyGroup chains",
	...
	polGroupEntry(
		...
		[]generictables.Rule{
			jumpToPolicyGroup(cali_pi_a, 0),
			jumpToPolicyGroup(cali_pi_b, 0x98),
		},
	),
	polGroupEntry(
		PolicyGroup{
		...
		[]generictables.Rule{
			jumpToPolicyGroup(cali_pi_a, 0),
			jumpToPolicyGroup(cali_pi_b, 0x98),
			jumpToPolicyGroup(cali_pi_c, 0x98),
		},
	),
	polGroupEntry(
		PolicyGroup{
		...
		[]generictables.Rule{
			...
			jumpToPolicyGroup(cali_pi_b, 0x98),
			jumpToPolicyGroup(cali_pi_c, 0x98),
			jumpToPolicyGroup(cali_pi_d, 0x98),
		},
	),
	polGroupEntry(
		PolicyGroup{
		...
		[]generictables.Rule{
			...
			jumpToPolicyGroup(cali_pi_c, 0x98),
			jumpToPolicyGroup(cali_pi_d, 0x98),
			jumpToPolicyGroup(cali_pi_e, 0x98),
		},
	),
	polGroupEntry(
		PolicyGroup{
	...
	polGroupEntry(
		...
		[]generictables.Rule{
			...
			jumpToPolicyGroup(cali_po_b, 0x98),
			jumpToPolicyGroup(cali_po_c, 0x98),
			jumpToPolicyGroup(cali_po_d, 0x98),
			jumpToPolicyGroup(cali_po_e, 0x98),
			{
				Match:   Match().MarkNotClear(0x98),
				Action:  ReturnAction{},
				Comment: []string{"Return on verdict"},
			...
			jumpToPolicyGroup(cali_po_g, 0x98),
		},
	),
	polGroupEntry(
		PolicyGroup{
		...
		[]generictables.Rule{
			...
			jumpToPolicyGroup(cali_po_d, 0x98),
			jumpToPolicyGroup(cali_po_e, 0x98),
			jumpToPolicyGroup(cali_po_f, 0x98),
			jumpToPolicyGroup(cali_po_g, 0x98),
			{
				Match:   Match().MarkNotClear(0x98),
				Action:  ReturnAction{},
			...
			jumpToPolicyGroup(cali_po_i, 0x98),
		},
	),
	polGroupEntry(
		PolicyGroup{
