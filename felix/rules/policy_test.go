...
var _ = Describe("Protobuf rule to iptables rule conversion", func() {
	...
	DescribeTable(
		"Allow rules should only have NFLOG when policy is staged with flowlogs enabled",
		func(ipVer int, in *proto.Rule, expMatch string) {
			rrConfigNormal.FlowLogsEnabled = true
			renderer := NewRenderer(rrConfigNormal)
			rules := renderer.ProtoRuleToIptablesRules(in, uint8(ipVer),
	...
	DescribeTable(
		"Deny rules should only include NFLOG for LOGandACCEPT mode with a staged policy with flowlogs enabled",
		func(ipVer int, in *proto.Rule, expMatch string) {
			rrConfigNormal.FlowLogsEnabled = true
			rrConfigLogAndDrop := rrConfigNormal
			rrConfigLogAndDrop.ActionOnDrop = "LOGandDROP"
