...
const (
	// nonClusterHostsYamlFile is the default path to the YAML file describing non-cluster hosts.
	// This file is generated by Banzai orchestration during cluster provisioning and mounted
	// into the e2e test container. Override with the NON_CLUSTER_HOSTS_YAML env var.
	nonClusterHostsYamlFile = "/non-cluster-hosts.yaml"

	logCollectorSplunkTemplate = `apiVersion: operator.tigera.io/v1
kind: LogCollector
metadata:
  name: tigera-secure
spec:
  additionalStores:
	...
	logCollectorDefault = `apiVersion: operator.tigera.io/v1
kind: LogCollector
metadata:
  name: tigera-secure`

	logCollectorSplunkNonClusterOnlyPatch = `apiVersion: operator.tigera.io/v1
kind: LogCollector
metadata:
  name: tigera-secure
spec:
