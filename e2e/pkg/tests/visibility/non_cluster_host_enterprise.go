...
var _ = describe.EnterpriseDescribe(
	...
	func() {
		...
		BeforeEach(func() {
			...
			hostsYAML := config.NonClusterHostsYAML()
			Expect(hostsYAML).NotTo(BeEmpty(), "NON_CLUSTER_HOSTS_YAML config must be set for non-cluster host tests")
			...
			hostsConfig := getNonClusterHosts(hostsYAML)
		...
		Context("Host to Service Flowlogs e2e", func() {
			...
			It("Captures HTTP traffic from non-cluster host to a NodePort service with deny egress policy applied", func() {
				...
				By(fmt.Sprintf("creating deny egress policy for port %d", destPort))
		...
		Context("Log archival of non-cluster host flows", func() {
			...
			BeforeEach(func() {
				splunkManager = splunk.NewManager(f)
			...
			It("Capture only HTTP traffic from non-cluster hosts in splunk when host scope is set to non-cluster only", func() {
				...
				splunkManager.SetHostScope(operatorv1.HostScopeNonClusterOnly)
...
func getNonClusterHosts(yamlPath string) hostsConfig {
