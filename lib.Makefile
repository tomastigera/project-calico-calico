...
# This setup is used to download and install the 'crane' binary into ./bin/crane.
# Normalize architecture for go-containerregistry filenames
CRANE_ARCH = $(subst amd64,x86_64,$(BUILDARCH))
CRANE_URL = https://github.com/google/go-containerregistry/releases/download/$(CRANE_VERSION)/go-containerregistry_Linux_$(CRANE_ARCH).tar.gz

# Install crane binary into bin/
bin/crane:
	mkdir -p bin
	curl -sSfL --retry 5 $(CRANE_URL) | tar xz -C bin/ crane
