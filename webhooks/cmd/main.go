...
var (
	// Enterprise-specific flags here.
	auditLogPath string
	mcmAddr      string
	mcmCAType    string
	mcmSecret    string
...
func init() {
	...
	WebhookCommand.Flags().StringVar(&auditLogPath, "audit-log-path", "/var/log/calico/audit/tsee-audit.log", "File path to write admission audit logs to.")
	WebhookCommand.Flags().StringVar(&mcmAddr, "mcm-management-cluster-addr", "", "Management cluster address for MCM.")
	WebhookCommand.Flags().StringVar(&mcmCAType, "mcm-management-cluster-ca-type", "", "Management cluster CA type for MCM.")
	WebhookCommand.Flags().StringVar(&mcmSecret, "mcm-tunnel-secret-name", "voltron-tunnel-cert", "Management cluster tunnel secret name for MCM.")
	WebhookCommand.Flags().BoolVar(&multiTenant, "multi-tenant", false, "Whether multi-tenancy is enabled.")
}
...
func serveWebhookTLS(cmd *cobra.Command, args []string) {
	...
	registerHooks(context.Background(), cs, calico)
