...
var _ = Describe("CalicoCni", func() {
	...
	Describe("with calico-ipam enabled, after creating a container", func() {
		...
		checkIPAMReservation := func() {
			...
			handleID := utils.GetHandleID("net1", containerID, workloadName, "eth0")
		...
		It("a second ADD for the same container should be a no-op", func() {
			...
			Expect(err).ShouldNot(HaveOccurred())
			// Routes are generated by the IPAM plugin on first ADD but not on the no-op
			// second ADD (which skips IPAM). Nullify before comparing.
