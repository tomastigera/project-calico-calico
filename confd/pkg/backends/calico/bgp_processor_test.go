...
func TestProcessBFDConfig_Success(t *testing.T) {
	...
	bfdConfigJSON, _ := json.Marshal(bfdConfig)

	cache := map[string]string{
		"/calico/resources/v3/projectcalico.org/bfdconfigurations/default": string(bfdConfigJSON),
	}

	c := newTestClient(cache, nil)

	config := &types.BirdBGPConfig{}
	err := c.processBFDConfig(config)
	require.NoError(t, err)

	require.Len(t, config.BFDConfig, 2)

	// First interface
	assert.Equal(t, "eth*", config.BFDConfig[0].MatchPattern)
	assert.Equal(t, "10ms", config.BFDConfig[0].MinRxInterval)
	assert.Equal(t, "100ms", config.BFDConfig[0].MinTxInterval)
	assert.Equal(t, "60000ms", config.BFDConfig[0].IdleTxInterval) // 1m = 60000ms
	assert.Equal(t, 5, config.BFDConfig[0].Multiplier)

	// Second interface
	assert.Equal(t, "ens*", config.BFDConfig[1].MatchPattern)
	assert.Equal(t, "20ms", config.BFDConfig[1].MinRxInterval)
	assert.Equal(t, "200ms", config.BFDConfig[1].MinTxInterval)
	assert.Equal(t, "120000ms", config.BFDConfig[1].IdleTxInterval) // 2m = 120000ms
	assert.Equal(t, 3, config.BFDConfig[1].Multiplier)
}

func TestProcessBFDConfig_NoConfig(t *testing.T) {
	cache := map[string]string{} // No BFD config
	...
	assert.Len(t, config.BFDConfig, 0)
}
...
func TestProcessBFDConfig_NilIntervals(t *testing.T) {
	...
	require.Len(t, config.BFDConfig, 1)
	assert.Equal(t, "0ms", config.BFDConfig[0].MinRxInterval)
	assert.Equal(t, "0ms", config.BFDConfig[0].MinTxInterval)
	assert.Equal(t, "0ms", config.BFDConfig[0].IdleTxInterval)
}

func TestBuildPeerFromData_DirectlyConnected(t *testing.T) {
	originalNodeName := NodeName
	NodeName = "node-1"
	defer func() { NodeName = originalNodeName }()
	...
	require.Len(t, config.Peers, 1)
	assert.True(t, config.Peers[0].DirectlyConnected)
}

func TestBuildPeerFromData_GatewayMode(t *testing.T) {
	originalNodeName := NodeName
	NodeName = "node-1"
	defer func() { NodeName = originalNodeName }()
	...
	tests := []struct {
		...
		{name: "Gateway mode direct", gatewayMode: "direct"},
		{name: "Gateway mode recursive", gatewayMode: "recursive"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Len(t, config.Peers, 1)
			assert.Equal(t, tt.gatewayMode, config.Peers[0].GatewayMode)
		})
	}
}

func TestBuildPeerFromData_EnableBFD(t *testing.T) {
	originalNodeName := NodeName
	NodeName = "node-1"
	defer func() { NodeName = originalNodeName }()
	...
	require.Len(t, config.Peers, 1)
	assert.True(t, config.Peers[0].EnableBFD)
}

func TestBuildPeerFromData_LongLivedGracefulRestart(t *testing.T) {
	originalNodeName := NodeName
	NodeName = "node-1"
	defer func() { NodeName = originalNodeName }()
	...
	tests := []struct {
	...
	}{
		{
			...
			restartTime:                "300",
			expectedLLGR:               true,
			expectedGracefulRestart:    "",
			expectedLongLivedStaleTime: "300",
		},
		{
			name:                    "Regular graceful restart",
			restartMode:             "GracefulRestart",
			restartTime:             "120",
			expectedLLGR:            false,
			expectedGracefulRestart: "120",
		},
		{
			name:                    "No restart mode - default graceful restart",
			restartTime:             "60",
			expectedLLGR:            false,
			expectedGracefulRestart: "60",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.restartMode != "" {
				peerData["restart_mode"] = tt.restartMode
			}
			if tt.restartTime != "" {
				peerData["restart_time"] = tt.restartTime
			}
			peerDataJSON, _ := json.Marshal(peerData)
			...
			require.Len(t, config.Peers, 1)
			assert.Equal(t, tt.expectedLLGR, config.Peers[0].LongLivedGracefulRestart)
			assert.Equal(t, tt.expectedGracefulRestart, config.Peers[0].GracefulRestart)
			assert.Equal(t, tt.expectedLongLivedStaleTime, config.Peers[0].LongLivedStaleTime)
		})
	}
}

func TestBuildPeerFromData_ExternalNetwork(t *testing.T) {
	originalNodeName := NodeName
	NodeName = "node-1"
	defer func() { NodeName = originalNodeName }()
	...
	tests := []struct {
		...
		{
			name:                  "External network does not exist - peer skipped",
			externalNetworkExists: false,
			expectedPeerCount:     0,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.externalNetworkExists {
				...
				externalNetworkJSON, _ := json.Marshal(externalNetworkData)
				cache["/calico/resources/v3/projectcalico.org/externalnetworks/test-net"] = string(externalNetworkJSON)
			}
			...
			require.Len(t, config.Peers, tt.expectedPeerCount)

			if tt.expectedPeerCount > 0 {
				assert.Equal(t, tt.expectedExternalTable, config.Peers[0].ExternalNetworkTable)
			}
		})
	}
}
