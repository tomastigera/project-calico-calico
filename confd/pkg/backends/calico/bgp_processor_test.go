...
func TestPopulateNodeConfig_RouterIDComment(t *testing.T) {
	originalNodeName := NodeName
	NodeName = "node-1"
	defer func() { NodeName = originalNodeName }()
	...
	tests := []struct {
	...
	}{
		{
			...
			ipVersion:               4,
			routerIDEnv:             "",
			expectedRouterIDComment: "",
		},
		{
			name:                    "IPv6 - default router ID with comment",
			ipVersion:               6,
			routerIDEnv:             "",
			expectedRouterIDComment: "# Use IPv4 address since router id is 4 octets, even in MP-BGP",
		},
		{
			name:                    "IPv6 - hash router ID with comment",
			ipVersion:               6,
			routerIDEnv:             "hash",
			expectedRouterIDComment: "# Use IP address generated by nodename's hash",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cache := map[string]string{
				"/calico/bgp/v1/host/node-1/ip_addr_v4": "10.0.0.1",
				"/calico/bgp/v1/host/node-1/ip_addr_v6": "2001:db8::1",
				"/calico/bgp/v1/global/as_num":          "64512",
			}
			...
			config := &types.BirdBGPConfig{}
			err := c.populateNodeConfig(config, tt.ipVersion)
			require.NoError(t, err)

			assert.Equal(t, tt.expectedRouterIDComment, config.RouterIDComment)
		})
	}
}
