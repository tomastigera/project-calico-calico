...
func (c *client) GetBirdBGPConfig(ipVersion int) (*types.BirdBGPConfig, error) {
	...
	// Process BFD configuration
	if err := c.processBFDConfig(config); err != nil {
		logc.WithError(err).Warn("Failed to process BFD configuration")
		return nil, err
	}
...
func (c *client) buildPeerFromData(peer *backends.BGPPeer, prefix string, config *types.BirdBGPConfig, nodeClusterID string, ipVersion int) *types.BirdBGPPeer {
	...
	// Restart mode handling
	if peer.RestartMode == "LongLivedGracefulRestart" {
		result.LongLivedGracefulRestart = true
		if peer.RestartTime != "" {
			result.LongLivedStaleTime = peer.RestartTime
		}
	} else if peer.RestartTime != "" {
		result.GracefulRestart = peer.RestartTime
	}

	// BFD support
	result.EnableBFD = peer.EnableBFD
	...
	// Gateway mode
	if peer.GatewayMode != "" {
		result.GatewayMode = peer.GatewayMode
	}

	// External network table
	if peer.ExternalNetwork != "" {
		// Check if external network exists
		externalNetworkKey := fmt.Sprintf("/calico/resources/v3/projectcalico.org/externalnetworks/%s", peer.ExternalNetwork)
		if _, err := c.GetValue(externalNetworkKey); err == nil {
			// Format the table name using the template function
			tableName, err := template.ExternalNetworkTableName(peer.ExternalNetwork)
			if err == nil {
...
// processBFDConfig processes BFD configuration
func (c *client) processBFDConfig(config *types.BirdBGPConfig) error {
	bfdConfigKey := "/calico/resources/v3/projectcalico.org/bfdconfigurations/default"
	bfdConfigValue, err := c.GetValue(bfdConfigKey)
	if err != nil {
		// BFD configuration is optional
		return nil
	}
	...
	for _, iface := range bfdConfig.Spec.Interfaces {
		...
		config.BFDConfig = append(config.BFDConfig, types.BFDInterfaceConfig{
			MatchPattern:   iface.MatchPattern,
			MinRxInterval:  minRx,
			MinTxInterval:  minTx,
