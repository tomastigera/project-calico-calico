...
func TestAuthorizer(t *testing.T) {
	...
	newAuthorizer := func(
		...
		ctx := NewUserAuthContext(context.Background(), &user.DefaultInfo{Name: userName}, authorizer, k8sClient, "Bearer fake-token")
	...
	t.Run("cache", func(t *testing.T) {
		...
		ctx = NewUserAuthContext(context.Background(), &user.DefaultInfo{Name: "fake-user2"}, authorizer, ctx.KubernetesClient(), "Bearer fake-token")
		authorized, err := authorizer.Authorize(ctx, "lma.tigera.io", []string{"dns"}, &testCluster)
		require.NoError(t, err)
		require.True(t, authorized)
