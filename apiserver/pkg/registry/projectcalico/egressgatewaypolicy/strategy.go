...
func GetAttrs(obj runtime.Object) (labels.Set, fields.Set, error) {
	apiserver, ok := obj.(*calico.EgressGatewayPolicy)
	if !ok {
		return nil, nil, fmt.Errorf("given object is not an EgressPolicy")
	}
	return labels.Set(apiserver.Labels), EgressGatewayPolicyToSelectableFields(apiserver), nil
}

// MatchEgressGatewayPolicy is the filter used by the generic etcd backend to watch events
// from etcd to clients of the apiserver only interested in specific labels/fields.
func MatchEgressGatewayPolicy(label labels.Selector, field fields.Selector) storage.SelectionPredicate {
...
// EgressGatewayPolicyToSelectableFields returns a field set that represents the object.
func EgressGatewayPolicyToSelectableFields(obj *calico.EgressGatewayPolicy) fields.Set {
