...
func GetAttrs(obj runtime.Object) (labels.Set, fields.Set, error) {
	apiserver, ok := obj.(*calico.BFDConfiguration)
	if !ok {
		return nil, nil, fmt.Errorf("given object is not a BFDConfiguration")
	}
	return labels.Set(apiserver.Labels), BFDConfigurationToSelectableFields(apiserver), nil
}

// MatchBFDConfiguration is the filter used by the generic etcd backend to watch events
// from etcd to clients of the apiserver only interested in specific labels/fields.
func MatchBFDConfiguration(label labels.Selector, field fields.Selector) storage.SelectionPredicate {
...
// BFDConfigurationToSelectableFields returns a field set that represents the object.
func BFDConfigurationToSelectableFields(obj *calico.BFDConfiguration) fields.Set {
