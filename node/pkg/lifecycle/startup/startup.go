...
func Run(opts ...RunOpt) {
	...
	log.Infof("Starting node %s with version %s (release version %s)", nodeName, buildinfo.Version, buildinfo.EnterpriseReleaseVersion)
...
func waitForReady(timeout time.Duration) error {
	...
	for {
		...
		default:
			if err := health.RunOutput(checkBIRD, checkBIRD6, checkFelix, false, false, false, false, 5*time.Minute); err != nil {
...
func configureBGPLayout(node *internalapi.Node) (bool, error) {
...
func configureASNumber(node *internalapi.Node, asStr string, source string) bool {
	// If Calico is running in policy only mode we don't need to write BGP related
...
func ensureDefaultConfig(
	...
	if err := c.EnsureInitialized(ctx, buildinfo.OpenSourceBaseVersion, buildinfo.EnterpriseReleaseVersion, clusterType); err != nil {
		return err
	}
